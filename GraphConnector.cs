    ...
    public class GraphConnector
    {
        ...
        public void Connect(RandomizerOptions opt, Graph g, AnnotationData ann)
        {
            ...
            while (tries++ < 100)
            {
                ...
                // Swap thos edges
                if (victimEdge != null & toFind != null)
                {
                    g.SwapConnectedEdges(victimEdge, toFind);
                }
                pairedOnly = !opt["unconnected"];
            }
            if (check == null || check.Unvisited.Count > 12) throw new Exception($"Couldn't solve seed {opt.DisplaySeed} - try a different one");
